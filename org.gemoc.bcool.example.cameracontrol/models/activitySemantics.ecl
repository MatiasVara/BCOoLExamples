...
ECLimport "platform:/resource/org.gemoc.bcool.example.cameracontrol/models/lib4RT.ccslLib"
...
--		    	(Relation Causes(lastFinalNode,self.finishActivity))
	...
	context MergeNode
	 	...
	 	inv MergeCausalitiesforTwoActions: 
	 	 ...
	 	 inv MergeCausalitiesforThreeActions: 
			 ((self.incoming->asSequence().source->select(n| (n).oclIsKindOf(Action))->size()=3)) implies	
			let unionOfIncomesB : Event = Expression Union(self.incoming.source.oclAsType(Action).finishAction) in
	 		(Relation Precedes(unionOfIncomesB, self.startIt))
